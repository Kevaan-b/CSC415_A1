...
def get_args():
    ...
    parser.add_argument("--pin_memory", action="store_true", help="Enable pinned host memory for DataLoader")
...
class Trainer:
    ...
    def do_training(self):
        ...
        for self.current_epoch in range(self.args.epochs):
            self._do_epoch()
            cur_val = float(self.results["val"][self.current_epoch].item())
            cur_test = float(self.results["test"][self.current_epoch].item())
            if cur_val > best_val:
                best_val = cur_val
                best_test = cur_test
        ...
        self._save_checkpoint(
            ...
            float(val_res[idx_best].item()),
            float(test_res[idx_best].item()),
        )
        message = "Best val %.5f (epoch: %d), corresponding test %.5f\n" % (best_val, best_epoch, best_test)
        print(message)
        with open(self.log_file, "a") as fid:
            fid.write(message)
